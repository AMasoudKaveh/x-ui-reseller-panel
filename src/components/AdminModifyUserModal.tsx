...
import {
  ...
  getAdminClientInbounds,
  modifyAdminClient,
  type AdminClientInbound
...
function inboundLabel(inbound: AdminClientInbound): string {
  return String(inbound.name || inbound.label || inbound.remark || `Inbound #${inbound.id}`);
}

export default function AdminModifyUserModal({ open, userId, onClose, onSaved }: Props) {
  ...
  const [inbounds, setInbounds] = useState<AdminClientInbound[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([getAdminClientDetail(userId), getAdminClientInbounds(userId)])
      .then(([detail, rows]) => {
        if (!alive) return;
      ...
      .catch(err => alive && setError(err instanceof Error ? err.message : "Unable to load client"))
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const rows = await getAdminClientInbounds(userId);
        if (!alive) return;
        setInbounds(rows);
        // Preserve currently attached IDs while x-ui/permission changes settle.
        setSelected(current => current.filter(id => rows.some(item => item.id === id)));
      } catch {
        // Keep last successful live list on a temporary x-ui error.
      }
    };
  ...
  const visibleInbounds = useMemo(() => {
    ...
    return inbounds.filter(item => [
      inboundLabel(item), item.protocol, item.network, item.security, item.port, item.id
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Update client failed");
    } finally {
      setSaving(false);
    }
  };
      ...
      <form className="xcu-modal" onSubmit={submit}>
        ...
        <div className="xcu-content">
          ...
          <aside className="xcu-inbounds">
            ...
            <button type="button" className="xcu-select-all" onClick={()=>setSelected(selected.length === inbounds.length ? [] : inbounds.map(x=>x.id))} disabled={loading || saving || !inbounds.length}>
            ...
            <div className="xcu-inbound-list">
              {loading ? <div className="xcu-loading"><LoaderCircle size={19} className="xcu-spinner"/>Loading...</div> : visibleInbounds.map(inbound => {
                ...
                return <button type="button" key={inbound.id} className={`xcu-inbound-item ${checked ? "selected" : ""}`} onClick={()=>toggleInbound(inbound.id)}>
                  ...
                  <div><strong>{inboundLabel(inbound)}</strong><small>{details}</small></div><span className="xcu-inbound-id">#{inbound.id}</span>
                </button>;
              })}
            </div>
          </aside>
